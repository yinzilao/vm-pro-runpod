...
class Simple3DPose(nn.Module):
    ...
    def forward(self, x, trans_inv, intrinsic_param, joint_root, depth_factor, flip_item=None, flip_output=False, flip_mask=None, return_feat=False, is_train=True, given_uvd_jts=None, given_confidence=None):
        """Forward pass
        Inputs:
        ...
        """
        if given_uvd_jts is None:

            batch_size = x.shape[0]
            preact_out = self.preact(x, return_feat=return_feat)     
            if return_feat:
                x0, features = preact_out   # (B, (J+K)*D, H, W), (B, (J+K), H, W)
            else:
                x0 = preact_out     # (B, (J+K)*D, H, W)
            ...
            batch_size = given_uvd_jts.shape[0]
            pred_uvd_jts = given_uvd_jts
            confidence = given_confidence
            heatmaps = None
        ...
        output = {
            'pred_xyz_jts': pred_xyz_jts,
            'confidence': confidence,
            'pred_uvd_jts_flat': pred_uvd_jts_flat,
            'pred_root_xy_img': pred_root_xy_img,
            'heatmaps': heatmaps
        }
        if return_feat:
            output.update({
